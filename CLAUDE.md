...
| `jlib/net` | `libjnet` | Email client stack: `Email` (MIME), `MailBox`/`MBox`/`Imap4Box`, `Pop3`, `Imap4`, `MailFolder`, plus `AS*` async variants layered on `sys::ASServent`. The protocol grammars live here — `rfc5322.hh` (addresses, appended to util's lexical block) read by `address.{hh,cc}`, and `rfc3501.hh` (IMAP responses) read by `imap_response.{hh,cc}`. `imap::read()` follows literals, which is why a response is not a line -- and has a suspending twin that agrees with it, including about what it refuses; `imap::quote()` writes command arguments. `http.{hh,cc}` is the client -- still deliberately narrow, and still saying so. `http_server.{hh,cc}` **was** narrow and is not any more: nine branches turned a loopback harness into a small HTTP/1.1 server. It does keep-alive (async only -- an idle connection is a suspended coroutine here and would be a held thread in the blocking one), chunked output, route patterns with captures and a most-specific-wins rule, 405 with `Allow`, HEAD answered by the GET route, conditional requests against `ETag`/`Last-Modified`, byte ranges for any body, and `files()` for serving a directory. Two things made that safe rather than reckless: `util::http::decide_framing` already refused both smuggling primitives, so keep-alive framing was never the open question it looked like; and every one of these is decided by the server rather than the handler, so a handler written for the harness still works. What it is **not** is hardened for a public port, which the header says at more length than it used to, because a server that routes and serves a directory is easier to point at the internet by mistake than one that answered a redirect; `oauth.{hh,cc}` is OAuth2: the refresh grant, the authorization-code grant with PKCE, a loopback `redirect_receiver`, and the XOAUTH2 message that carries a token to IMAP. |
...
- `jserve` — an OpenAI-compatible endpoint over `ai::engine` and
  ...
  **aider works against it end to end**, in its `whole` edit format, which is
  the one that needs no tool calling. Asynchronous since #218: tokens stream
  as chunked SSE off a `sys::relay`, and a generation runs on its own thread
  rather than on the reactor or in the request pool. Loopback only, and the
  header says why.
...
**jlib waits on more than one thing at once now, which it never could.** #4 was
open from the CVS import asking for "something like boost asio io_service", and
the problem was never speed -- it was that nothing composed. There was exactly
one multi-descriptor `poll()` in the tree and four unrelated ways to wait:
`Servent` on a pipe, `ASServent` on a condition variable, `job_queue` on a
queue depth, `server` on `poll(2)`. No thread could wait for a request *and* a
socket.

The fork -- stackful fibers or C++20's stackless coroutines -- was decided
against fibers on measured grounds and recorded on #4: ASan needs annotations
around every context switch and caught a real use-after-free in the reactor's
dispatch loop during the work; Boost.Context is ~2000 lines of per-platform
assembly against a library with four dependencies; and the colouring cost of
stackless turned out to be **six framing functions**, not fifty-nine, because
the RFC-grammar work had already separated framing from parsing for unrelated
reasons.

What that bought, in order: a reactor, `sys::server` on it, `task<T>` and the
awaitables, the six framing functions, async TLS, a cancel that ends a wait,
an async server, and an HTTP server that runs **the same buffered handlers as
the blocking one** -- which is how the two are compared, request by request, on
one route table.

**What it deliberately did not do is migrate anything.** `net::http`, `Imap4`,
`Pop3` and `oauth` are all still blocking, every blocking path is untouched,
and `run_until_complete` exists so that a synchronous caller stays synchronous.
A mail client with one connection blocking in its own thread is fine; the
machinery is there for the things that are not.

**`jserve` is the exception, and it was migrated later and on purpose** (#218).
It is the one thing here that wanted what async bought: a coding harness makes
many requests, and it had been paying a TCP and TLS handshake for each. Its
token loop is the shape the architecture asks for -- inference on a thread of
its own, reporting back through a `sys::relay`, written on the reactor -- and
`ai::generate` taking a callback is what forced that, because **a callback
cannot `co_await`**.

Three habits from that work worth keeping, beyond the three below. **A test
that passes on the broken code is not a test** -- every conversion was verified
by breaking the thing it guards and watching it fail, which caught a STARTTLS
assertion that stayed green without the fix and a TLS test that was green for
the wrong reason twice. **Say which assertion is load-bearing**, because the
weak one usually looks essential. And **the container earns its keep**: it
caught a missing `$(OPENSSL_LIBS)`, fifteen `-Wunused-result` warnings, and an
include of `url.hh` where the file is `URL.hh` -- invisible on a
...
**And then the server grew.** Nine branches after #4 closed, in this order and
each because the one before it made the next worth having: keep-alive (which
found three latent defects in `sys` that only a reused connection could reach —
a missing `TCP_NODELAY` worth ~44ms per TLS connection, an unguarded
`cancel_token`, and `stop()` tearing connections down on the caller's thread);
chunked output; `jserve` converted to async; route patterns with 405; HEAD and
conditional requests; a static file handler; streamed file bodies; and byte
ranges.

Two habits from that arc, beyond the three below. **Break the guard, and when
it will not break, ask why** — four separate checks turned out to be dead code,
redundant with a check beside them, or guarding something no test could reach,
and each answer was different. And **a test harness can lie**: one reported
"still passes" for a build that never compiled, another for a test that
crashed before reaching an assertion, a third for a patch whose anchor did not
match, and a container suite once reported 85 passes from a stale image after
`docker build` failed. Every one of them was caught by checking an exit status
