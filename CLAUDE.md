...
| `jlib/net` | `libjnet` | Email client stack: `Email` (MIME), `MailBox`/`MBox`/`Imap4Box`, `Pop3`, `Imap4`, `MailFolder`, plus `AS*` async variants layered on `sys::ASServent`. The protocol grammars live here — `rfc5322.hh` (addresses, appended to util's lexical block) read by `address.{hh,cc}`, and `rfc3501.hh` (IMAP responses) read by `imap_response.{hh,cc}`. `imap::read()` follows literals, which is why a response is not a line -- and has a suspending twin that agrees with it, including about what it refuses; `imap::quote()` writes command arguments. `http.{hh,cc}` is the client and `http_server.{hh,cc}` the server, both deliberately narrow and both saying so -- though the async server now answers **more than one request per connection**, which the blocking one deliberately does not: an idle persistent connection is a suspended coroutine here and would be a held thread there. What made keep-alive safe enough to add was already in `util::http` -- `decide_framing` refuses both smuggling primitives -- plus the rule that any framing error closes rather than looking for the next request in a stream it has just said it cannot parse; `oauth.{hh,cc}` is OAuth2: the refresh grant, the authorization-code grant with PKCE, a loopback `redirect_receiver`, and the XOAUTH2 message that carries a token to IMAP. |
...
`Pop3`, `oauth` and `jserve` are all still blocking, every blocking path is
untouched, and `run_until_complete` exists so that a synchronous caller stays
synchronous. A mail client with one connection blocking in its own thread is
fine; the machinery is there for the things that are not.
