...
int main(int argc, char** argv) {
    ...
    std::unique_ptr<cuda::NeuralNetwork<T>> nn;
    
    if(load_file.empty()) {
        nn.reset(new cuda::NeuralNetwork<T>(train_rate, INODES, HNODES, ONODES));
    } else {
        ...
        nn.reset(new cuda::NeuralNetwork<T>(o));
    }
